...
double dirichlet_node(SEXP target, SEXP x, SEXP data, SEXP iss, int per_cell,
    SEXP prior, SEXP beta, SEXP beta_iss, SEXP experimental, int sparse,
    bool debugging) {
...
SEXP nodes, node_t, data_t, exp_data, parents, parent_vars, config;
  ...
  /* FILIPs addition */
  double prior_iss = NUM(beta_iss);

  if (debugging) Rprintf("  > (log)prior probability before is %lf.\n", prior_prob);
  prior_prob *= prior_iss;
  if (debugging) Rprintf("  > (log)prior probability after is %lf.\n", prior_prob);
  /* End FILIPs addition */
  ...
  prob += prior_prob;
  if (debugging) Rprintf("  > (log)prior + (log)posterior is %lf.\n", prior_prob);
