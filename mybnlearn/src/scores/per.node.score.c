...
SEXP cur, iss, prior, beta, beta_iss, exp, l, nu, iss_w, newdata, custom_fn, custom_args;
  ...
  switch(s) {
    ...
    case BDS:
      ...
      beta_iss = getListElement(extra_args, "beta_iss");
      ...
      for (i = 0; i < ntargets; i++) {
        ...
        res[i] = dirichlet_node(cur, network, data, iss, FALSE, prior, beta,
                   beta_iss, R_NilValue, (s == BDS), debugging);
    ...
    case K2:
      for (i = 0; i < ntargets; i++) {
        ...
        res[i] = dirichlet_node(cur, network, data, iss, TRUE,
                   R_NilValue, R_NilValue, R_NilValue, R_NilValue, FALSE,
                   debugging);
